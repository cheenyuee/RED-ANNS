...
inline int tcp_base_port;
inline int num_rc_qps;
inline int num_uc_qps;
inline int num_ud_qps;
inline int node_id;

inline std::vector<std::string> network;

// per-thread allocator
inline __thread RdmaDevice **rdma_devices_;
...
public:
    ...
    int tid = 0;    // 表示这个 qp 属于本地哪个线程
    int nid = 0;    // 表示这个 qp 与远端哪个节点通信
    int idx_ = 0;   // 表示这个 qp 的 index
    int port_idx;   // 表示这个 qp 的 port index
    int pendings = 0; // 表示当前已提交但未完成的操作请求
    ...
    bool connect_rc() {
        ...
        /* 处理 reply_buf 中接收到的数据 */

        // 根据 reply_buf 的 header 判断是否传输成功
        ...
        // 根据 reply_buf 的 body 读取远端 qp 信息
        RdmaQpAttr qp_attr;
    ...
    void change_qp_states(RdmaQpAttr *remote_qp_attr, int dev_port_id) {
        ...
        if (qp->qp_type == IBV_QPT_RC) {
            ...
            rc_init2rtr(qp, dev_port_id, remote_qp_attr->qpn, remote_qp_attr->lid, remote_qp_attr->gid); // RoCE need remote port GID
            rc_rtr2rts(qp);
    ...
    inline IOStatus rc_post_send(ibv_wr_opcode op, char *local_buf, int len, uint64_t off, int flags, int wr_id = 0) {
    ...
    inline IOStatus rc_post_doorbell(RdmaReq *reqs, int batch_size) {
    ...
    /*
     * 如果使用无信号完成，则有且只有最后一个 sr 的 send_flags 为 IBV_SEND_SIGNALED
     */
    inline IOStatus rc_post_doorbell(RdmaReq *reqs, int batch_size, bool use_unsignal) {
        ...
        for (uint i = 0; i < batch_size; i++) {
            ...
            if (use_unsignal && reqs[i].flags != 0)
                logstream(LOG_ERROR) << "flags value error." << LOG_endl;

            if (use_unsignal && i == batch_size - 1) {
                sr[i].send_flags |= IBV_SEND_SIGNALED;
            }
    ...
    inline IOStatus poll_completion(uint64_t *rid = NULL) {
    ...
    inline IOStatus poll_completions(int cq_num, uint64_t *rid = NULL) {
            ...
            for (int cq_id = 0 ; cq_id < cq_num; cq_id ++) {
                if (wc[cq_id].status != IBV_WC_SUCCESS) {
                    fprintf (stderr,
                             ...
                             wc[cq_id].status, wc[cq_id].vendor_err, ibv_wc_status_str(wc[cq_id].status));
                    assert(false);
                    return IO_ERR;
                    // exit(-1);
                }
    ...
    /*
     * 返回true时，意味着此时没有未完成发送请求
     */
    inline bool first_send() { return pendings == 0; }

    /*
     * 返回true时，意味着此时已经有足够多的未完成发送请求，需要进行轮询完成队列以处理这些请求并重置pendings
     */
    ...
    /*
     * force_poll 函数将成员变量 pendings 设置为预定义的阈值 POLL_THRSHOLD，这样做的目的是在下次发送操作时，强制触发对完成队列的轮询操作
     */
...
public:
    ...
    void open_device(int dev_id = 0) {
        ...
        logstream(LOG_INFO) << "open divce: " << device->name << LOG_endl;
    ...
    static void* recv_thread(void *arg) {
        ...
        try {
            while (running) {
                ...
                // 读取远端的 QPConnArg 信息
                uint64_t qid = arg.qid; // 要连接的 qp id
    ...
    Qp *create_rc_qp(int tid, int remote_id, int dev_id, int port_idx, int idx = 0) {
        ...
        res->init_rc(get_rdma_device(dev_id), port_idx); // 指定 qp 所使用的 device（之前已打开的 device）
    ...
    /* 同时没有修改qps_则不需要加锁 */
    inline Qp *get_rc_qp_nolock(int tid, int remote_id, int idx = 0) {
        // mtx_->lock();
        uint64_t qid = (uint64_t)(_QP_ENCODE_ID(remote_id, RC_ID_BASE + tid * num_rc_qps_ + idx));
        // fprintf(stdout,"find qp %d %d %d, qid %lu\n",tid,remote_id,idx,qid);
        assert(qps_.find(qid) != qps_.end());
    ...
    RdmaQpAttr get_local_qp_attr(int qid) {
        ...
        // 获取 RoCE 协议需要的 GID 信息
        int default_gid_idx = 0;
        if (ibv_query_gid(local_qp->qp->context, local_qp->port_idx, default_gid_idx, &qp_attr.gid)) {
			fprintf(stderr, "can't read gid of index %d\n", default_gid_idx);
			assert(false);
		}
...
inline bool Qp::get_ud_connect_info_specific(int remote_id, int thread_id, int idx) {
...
/*
 * 在 RoCE 环境下，ibv_modify_qp 使用全局路由则需要远端的 GID 信息
 */
static void rc_init2rtr(ibv_qp * qp, int port_id, int qpn, int dlid, union ibv_gid dgid) {
    int rc, flags;
    struct ibv_qp_attr qp_attr;
    ...
    qp_attr.dest_qp_num = qpn;
    qp_attr.rq_psn = DEFAULT_PSN;
    qp_attr.max_dest_rd_atomic = 16; // 最大并发响应数
    qp_attr.min_rnr_timer = 12;
    ...
    {
        int default_sgid_idx = 0;
		qp_attr.ah_attr.is_global = 1;
		qp_attr.ah_attr.grh.hop_limit = 1;
		qp_attr.ah_attr.grh.dgid = dgid;
		qp_attr.ah_attr.grh.sgid_index = default_sgid_idx;
	}
    else
    {
        logstream(LOG_ERROR) << "can't find dgid in RoCE." << LOG_endl;
    }
...
static void rc_rtr2rts(ibv_qp * qp) {
    ...
    qp_attr.retry_cnt = 7;
    qp_attr.rnr_retry = 7;
    qp_attr.max_rd_atomic = 16; // 最大同并发请求数
