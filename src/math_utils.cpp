...
#pragma omp parallel for schedule(static, 8192)
    ...
    void rotate_data_randomly(float* data, size_t num_points, size_t dim, float* rot_mat, 
...
#pragma omp critical
    ...
    uint32_t* closest_centers = new uint32_t[PAR_BLOCK_SIZE];
    float*    distance_matrix = new float[num_centers * PAR_BLOCK_SIZE];
...
#pragma omp parallel for schedule(static, 1)
        for (int64_t j = cur_blk * PAR_BLOCK_SIZE;
           j < std::min((int64_t) num_points, (int64_t)((cur_blk + 1) * PAR_BLOCK_SIZE));
           j++) 
        {
            size_t this_center_id = closest_centers[(j - cur_blk * PAR_BLOCK_SIZE)];
            closest_centers_ivf[j] = (uint32_t) this_center_id;
            float this_closest_distance = distance_matrix[this_center_id + num_centers * (j - cur_blk * PAR_BLOCK_SIZE)];
            closest_distance[j] = this_closest_distance;
            if (inverted_index != NULL) {
#pragma omp critical
                inverted_index[this_center_id].push_back(j);
            }
...
#pragma omp parallel for schedule(static, 8192)
        for (int64_t n_iter = 0; n_iter < (int64_t) num_points; n_iter++) {
            for (size_t d_iter = 0; d_iter < dim; d_iter++) {
                if (to_subtract == 1)
                    data_load[n_iter * dim + d_iter] = data_load[n_iter * dim + d_iter] - cur_pivot_data[closest_centers[n_iter] * dim + d_iter];
                else
...
#pragma omp parallel for schedule(static, 8192)
            for (int64_t i = 0; i < (int64_t) num_points; i++) {
                dist[i] = (std::min)(
                    dist[i], math_utils::calc_distance(data + i * dim, data + tmp_pivot * dim, dim));
            }
            num_picked++;
        }
        delete[] dist;
