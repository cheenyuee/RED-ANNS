...
inline vector<vector<int>> cpu_topo;
inline int num_cores = 0;

inline bool enable_binding = false;
inline vector<int> default_bindings; // bind to core one-by-one
inline map<int, int> core_bindings; // user-defined core binding


inline void dump_node_topo(vector<vector<int>> topo)
{
...
inline void load_node_topo(void)
{
...
inline bool load_core_binding(string fname)
{
    int nbs = 0;
 ...
 */
inline cpu_set_t get_core_binding()
{
    cpu_set_t mask;
    CPU_ZERO(&mask);
 ...
 */
inline cpu_set_t unbind_to_core()
{
    cpu_set_t mask;
