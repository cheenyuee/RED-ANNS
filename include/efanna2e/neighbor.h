   ...
   public:
    ...
    bool insert(const Neighbor &nbr) {
        if (_size == _capacity && _data[_size - 1] < nbr) {
            // std::cout << "Reaching capacity or Encountering bs node#" << nbr.id << std::endl;
            return false;
        }
        ...
        // std::cout << "node#" << nbr.id << "inserted!" << std::endl;
    ...
    Neighbor closest_unexpanded() {
        ...
        // std::cout << "node#" << _data[pre].id << " popped!" << std::endl;
        return _data[pre];
    }
    ...
    float get_threshold() { return _data[_size].distance; }
    
    void print_queue(){
        std::cout << "[ ";
        for (const auto& neighbor : _data){
    ...
    // Get the IDs of the closest n unexpanded nodes
    std::vector<uint32_t> get_closest_unexpanded_ids(size_t num) {
        std::vector<uint32_t> closest_ids;
        size_t count = 0;
        size_t start = _cur;

        while (start < _size && count < num) {
            if (!_data[start].flag) { // Check if the node is unexpanded
                closest_ids.push_back(_data[start].id);
                count++;
            }
            start++;
        }
